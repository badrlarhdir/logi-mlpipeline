...
class ReportBuilder:
    ...
    def set_notebooks(self, notebooks : list[str], subfolder: str = None):
        ...
        for notebook in notebooks:
            ...
            cells = list(filter(lambda cell: "report." in cell.source, nb.cells))
    ...
    def __save_report(self, subfolder: str = None):
        ...
        with open(os.path.join(os.path.dirname(__file__), 'base.yaml'), "r") as base_f:
        ...
        data_self_hosted_runner["on"] = {
            "repository_dispatch": {
                "types": ["mlpipeline-main" if not subfolder else f"mlpipeline-{subfolder}"],
            }
        }
        data_self_hosted_runner["jobs"]["pipeline"]["steps"][-1]["run"] = self.__report_cmds
        ...
        if subfolder:
            ...
            if pathlib.Path(f'{PIPELINES_FOLDER}/{subfolder}/outputs').exists():
                shutil.rmtree(f'{PIPELINES_FOLDER}/{subfolder}/outputs')
            shutil.copytree('./outputs', f'{PIPELINES_FOLDER}/{subfolder}/outputs')

            # Copy the dvc.lock file to the pipeline folder
            if pathlib.Path('./dvc.lock').exists():
                shutil.copy('./dvc.lock', f'{PIPELINES_FOLDER}/{subfolder}/dvc.lock')
            # Copy the .dvcignore file to the pipeline folder
            shutil.copy('./.dvcignore', f'{PIPELINES_FOLDER}/{subfolder}/.dvcignore')

            # Set the name of the GHA jobs
            data_self_hosted_runner["name"] = f"{subfolder} Self Hosted Runner ML-pipeline"

            # Set the working directory for the self hosted runner to 
            # the pipeline folder
            data_self_hosted_runner["jobs"]["pipeline"]["defaults"] = {
                "run": { 
                    "working-directory": f'{PIPELINES_FOLDER}/{subfolder}'
                }
            }
        ...
        if subfolder:
            gh_workflows_path = pathlib.Path(f'./{PIPELINES_FOLDER}/{subfolder}/.github/workflows')
        ...
        if not subfolder:
            ...
            with open(gh_workflows_path / 'self-hosted-runner.yaml', 'w') as self_hosted_runner_f:
                yaml.dump(data_self_hosted_runner, self_hosted_runner_f, sort_keys=False)
        else:
            # Copy the self-hosted-runner.yamlto the subfolder repo
            with open(gh_workflows_path / f'{subfolder}-self-hosted-runner.yaml', 'w') as self_hosted_runner_f:
                yaml.dump(data_self_hosted_runner, self_hosted_runner_f, sort_keys=False)
            # Copy the file to the main repo too
            with open(f'./.github/workflows/{subfolder}-self-hosted-runner.yaml', 'w') as self_hosted_runner_f:
                yaml.dump(data_self_hosted_runner, self_hosted_runner_f, sort_keys=False)
