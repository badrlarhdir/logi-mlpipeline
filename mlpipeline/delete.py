...
def delete_default_pipeline(pipeline: str = None):
    ...
    with open(f"{PIPELINES_FOLDER}/pipelines.json", 'r') as pipelines_f:
        pipelines = json.load(pipelines_f)
        if (pipeline and pipelines[DEFAULT_PIPELINE] == pipeline) or (not pipeline):
            pipelines[DEFAULT_PIPELINE] = ''
            with open(f"{PIPELINES_FOLDER}/pipelines.json", 'w') as pipelines_f:
                json.dump(pipelines, pipelines_f)
...
def delete(pipeline: str, all: bool):
    ...
    if pipeline:
        ...
        return
    print("Please specify a pipeline name using -p or use the --all or -a flag to delete all pipelines")