...
def cloud_status(pipeline: str):
    ...
    repo = subprocess.check_output(['git', 'config', '--get', 'remote.origin.url']).decode('utf-8').strip()
    repo = repo.split('/')[-2:]
    ...
    response = [x for x in response['workflow_runs'] if x['display_title'] == f'mlpipeline-{pipeline}']
    ...
    table_result = [
        ...
        [response[0]["id"], response[0]["head_branch"], response[0]["actor"]['login'], response[0]["status"], response[0]["conclusion"]]
    ]
