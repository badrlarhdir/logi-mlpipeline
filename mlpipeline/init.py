...
def init():
    ...
    if not pathlib.Path('.gitignore').exists():
        ...
        shutil.copyfile(os.path.join(os.path.dirname(__file__), '../tests/resources/.gitignore.txt'), '.gitignore')
    ...
    if not pathlib.Path('./data/.gitignore').exists():
        with open('./data/.gitignore', 'w') as gitignore_f:
            gitignore_f.write('# Ignores all data files in the data folder\n# Except the .dvc extension files\n')
