...
def test_sync_notebooks_empty_argument():
    ...
    assert "Please specify a pipeline name to sync or a list of notebooks" in result.output
...
def test_sync_on_unexisting_notebooks():
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ['sync', '-n', '[nbs/preprocessing.ipynb, nbs/training.ipynb]'])
...
def test_sync_on_existing_notebooks():
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ['sync', '-n', '[notebooks/data_preprocess.ipynb, notebooks/train.ipynb]'])
